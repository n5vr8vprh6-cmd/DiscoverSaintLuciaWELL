...
const { profileFor, savePersona } = require('../gtm.js');
...
function revealScreen(res, advisor, profile, answers, done) {
  ...
  <div class="wrap wrap--narrow">
      ...
      <label class="hub-field">
        ...
        <select name="primary">
          ${profiles.map((x) => `<option value="${esc(x.key)}"${
            x.key === shownPrimary ? ' selected' : ''}>${esc(x.name)} — ${esc(x.advantage)}</option>`).join('')}
        </select>
      </label>
    ...
    <div class="hub-actions">
      <a class="btn btn--gold" href="/hub/campaign">Build my plan</a>
