...
/* Unlimited, and the balance is not even looked at. Kept as one function so
   ...
   ── PAID COUNTS HERE. TRAINED COUNTS ON THE LADDER. ───────────────────────
   `foundations_paid_at` (021) is the entitlement: somebody bought Foundations,
   so the meter stops the moment the webhook lands rather than whenever an
   admin next looks. `foundations_at` is the qualification: they completed the
   training, and it is what rung() reads to decide whether their copy may say
   "trained in the Well Destination method".

   THE TWO MUST NOT BE MERGED BACK. It will be tempting — one date is simpler
   than two, and payment is nearly always followed by attendance. But merging
   them means a person who buys the programme and never turns up can publish a
   claim to be trained, under their own name, generated by us. That is the
   exact failure the claims checker exists to prevent, and it would arrive as a
   convenience.

   rung() in gtm.js reads the training dates and NOT this one. That asymmetry
   is the feature. tools/builds-test.js asserts it in those words. */
function unmetered(advisor) {
  return Boolean(advisor && (
    advisor.foundations_paid_at || advisor.foundations_at || advisor.immersion_at));
}
...
/* ── Foundations, paid for ────────────────────────────────────────────────
   Sets or clears foundations_paid_at, and touches NOTHING else. No builds are
   granted alongside it: the meter is off for this advisor now, so a balance is
   a number nobody reads.

   IT DOES NOT SET foundations_at, AND MUST NEVER BE MADE TO. That column says
   the person completed the training and unlocks what their copy may claim
   about them — see the note on unmetered() above, and 021. A webhook knows
   that money moved. It does not know whether anybody turned up.

   A plain UPDATE rather than an RPC, because unlike spend_plan_build there is
   no read-modify-write to lose: this writes a constant. */
async function markFoundationsPaid(advisorId, on) {
  const supabase = db();
  if (!supabase || !advisorId) return { ok: false, reason: 'no_db' };

  const { data, error } = await supabase.from('advisors')
    .update({ foundations_paid_at: on ? new Date().toISOString() : null })
    .eq('id', advisorId)
    .select('id, foundations_paid_at, foundations_at')
    .maybeSingle();

  if (error) {
    if (isMissing(error)) return { ok: false, reason: 'not_migrated' };
    console.error('markFoundationsPaid', error);
    return { ok: false, reason: 'mark_failed' };
  }
  if (!data) return { ok: false, reason: 'no_such_advisor' };
  return { ok: true, paidAt: data.foundations_paid_at, trained: Boolean(data.foundations_at) };
}
