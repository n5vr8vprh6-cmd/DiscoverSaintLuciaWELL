...
{ id: 'G-33', role: 'guard', priority: 1, area: 'Webhook',
  title: 'Coming back from checkout never reads as a failure',
  needs: ['Return URL set on the ThriveCart product'],
  steps: ['Buy the pack and let ThriveCart return you to /hub/campaign?done=purchased',
          'Read the message the instant you land, before refreshing',
          'Refresh once the webhook has arrived'],
  expect: 'Either "3 more campaigns are on your account" or "we are confirming your purchase \u2014 refresh in a moment". Never an error, and never the plain word "purchased".',
  why: 'The browser redirect is immediate and the webhook is not, so an advisor can land here before the balance has moved. Their money has already gone: telling them something went wrong sends them to support over a delay that resolves itself in seconds.' },

{ id: 'G-34', role: 'guard', priority: 2, area: 'Webhook',
  title: 'An unknown ?done value renders no message',
  steps: ['Open /hub/campaign?done=nonsense'],
  expect: 'No flash at all. The word "nonsense" appears nowhere.',
  why: 'The fallback used to echo any token straight onto the page as though the product had written it. Escaped, so cosmetic rather than dangerous \u2014 but a confirmation message we did not write is not a confirmation.' },
...
{ id: 'G-21', role: 'guard', priority: 1, area: 'Webhook',
  ...
  expect: 'A row recorded with a note and a delta of 0. The balance does not move. The note reads "Sandbox build pack order \u2014 recorded so you can confirm the wiring, but no money moved so nothing was granted."',
...
{ id: 'A-64', pass: 'ASK WELL', role: 'advisor', priority: 2, area: 'ASK WELL',
...
{ id: 'A-72', pass: 'ASK WELL', role: 'advisor', priority: 1, area: 'ASK WELL',
  title: 'Understand — multi-select, four notes and a budget figure survive a reload and never leave the row',
  steps: ['On Understand type a line under "What they said about it" and press its gold Save; tick two things under "Why are you travelling" and two under "What would make you hesitate", including "Something else", and write in each Extra notes box',
          'Type a note under "Anything we should plan around"; type 18000 into "Roughly what feels right" with 7 nights set; tick "Open, if it is right" beside the figure; read the lines under it; then set the month to something before this month',
          'Reload; then open Shape and press "Draft a line"; then issue and open /j/:token'],
  expect: 'After the reload every tick, all four notes and the figure are still there. The line under the figure reads "reads as Premium · about $2,571 a night across the stay · a guide, not a ceiling"; under it, "The places we’re considering start from about $X for 7 nights in February … before flights"; a figure under that floor is called out in gold; a month the lookup does not cover says which months it does. The month picker refuses a past month. "What I heard" says it all back in the ink card, notes quoted with their labels. The drafted line and the client document contain none of the notes or the figure. With 025 not applied, only the "Extra notes" under why-now remains; with 024 not applied, one radio per question.',
  why: 'Duncan asked for room to be human — notes, a real number — and the privacy rule asked that prose never reach a prompt. Both hold because every note lives on the row and is never copied into a need-state. tools/design-privacy-test.js poisons all of them; this checks it on a live deployment.' },

{ id: 'A-74', pass: 'ASK WELL', role: 'advisor', priority: 2, area: 'ASK WELL',
  title: 'Understand — her own words first, the island that answers as you talk, the ink card, and the way to Compare',
  steps: ['From a Journey press "Design this journey" — it must open on Understand; read the quote card, then the line under the headline, then the sentence above the island',
          'Change the leading "moving away from" answer or the nights and let it save; watch the island band',
          'Read "What I heard" at the foot; press "Let’s compare the places →"; then set the OS to reduce motion and press it again; then disable JavaScript and press it a third time'],
  expect: 'Understand, every time, never Compare. A white card with a gold quotation mark holds the client’s own Finder answers — "You said you need ‘space to think clearly’, and ‘the rainforest’ called you first" — and "With <advisor> · <business>" sits under the headline; toward chips are tinted teal, away-from chips plain. Above the map: "Five places on the island answer moving from running hot toward steady and fully here — most of them in Nature & Renewal." After a save the sentence and the pins re-render without a reload and the hover card still works. "What I heard" is the one dark block on the page — ink card, gold rule, the paragraph large, the notes quoted, then "Ask: ‘Anything you feel is missing before we lay this out?’" and one gold button. The button shows the ring and three lines for about two seconds, then Compare; with reduced motion one still frame under a second; with JavaScript off, straight to Compare.',
  why: 'These are the concierge moves — affirm what they said in their words, name who is in the room, let the island answer as the conversation moves, close on the read-back and one clear next step. Each is theatre only if it lies; the quote is their words, the count is real, and the options were computed before the ring ever turned.' },
...
{ id: 'A-66', pass: 'ASK WELL', role: 'advisor', priority: 3, area: 'Playbook',
...
{ id: 'G-35', pass: 'ASK WELL', role: 'guard', priority: 1, area: 'ASK WELL',
...
{ id: 'G-36', pass: 'ASK WELL', role: 'guard', priority: 1, area: 'ASK WELL',
...
{ id: 'G-37', pass: 'ASK WELL', role: 'guard', priority: 1, area: 'ASK WELL',
...
{ id: 'G-38', pass: 'ASK WELL', role: 'guard', priority: 1, area: 'ASK WELL',
...
{ id: 'G-39', pass: 'ASK WELL', role: 'guard', priority: 2, area: 'ASK WELL',
...
{ id: 'G-41', pass: 'ASK WELL', role: 'guard', priority: 2, area: 'ASK WELL',
...
{ id: 'G-42', pass: 'ASK WELL', role: 'guard', priority: 1, area: 'ASK WELL',
...
{ id: 'G-44', pass: 'ASK WELL', role: 'guard', priority: 2, area: 'ASK WELL',
...
{ id: 'G-45', pass: 'ASK WELL', role: 'guard', priority: 2, area: 'Playbook',
...
{ id: 'G-46', pass: 'ASK WELL', role: 'guard', priority: 1, area: 'ASK WELL',
  title: 'A handed-over Journey takes its workspace with it',
  steps: ['As the house account, design a pooled Journey and issue a plan',
          'Introduce that Journey to UAT2',
          'As UAT2, open the Journey and press Design',
          'As the house account, try to open the same design and withdraw the issued plan'],
  expect: 'UAT2 sees the consultation, the session and the issued plan. The house account sees not found and cannot withdraw anything.',
  why: 'These tables carry their own advisor_id, so a handover that moves the Journey and not the workspace leaves the SENDING advisor able to withdraw a live document from a client who now belongs to somebody else — and the receiving advisor unable to open the work that was done for them.' },
{ id: 'T-07', pass: 'ASK WELL', role: 'teardown', priority: 1, area: 'Cleanup',
