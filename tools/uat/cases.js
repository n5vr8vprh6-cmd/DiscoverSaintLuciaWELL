...
{ id: 'A-72', pass: 'ASK WELL', role: 'advisor', priority: 1, area: 'ASK WELL',
  title: 'Understand — multi-select, four notes, a story and a budget figure survive a reload and never leave the row',
  steps: ['On Understand type a line under "What they said about it" and press its Save; tick two things under "Why are you travelling" and two under "What would make you hesitate", including "Something else", and write in each Extra notes box',
          'Type a note under "Anything we should plan around"; type 18000 into "Roughly what feels right" with 7 nights set; read the line under the figure',
          'Write a story on the island card ("Your story about this place") and Save; reload; open Compare and find the same story on that card; then open Shape and press "Draft a line"; then issue and open /j/:token'],
  expect: 'After the reload every tick, all four notes, the figure and the story are still there. The budget line reads "reads as Premium · about $2,571 a night across the stay"; under it, "The places we’re considering start from about $X for 7 nights in February … before flights"; a figure under that floor is called out in gold. "What I heard" says it all back, notes quoted with their labels. The drafted line and the client document contain none of the notes, the story or the figure. With 025 not applied, only the "Extra notes" under why-now remains and there are no story forms; with 024 not applied, one radio per question.',
  why: 'Duncan asked for room to be human — notes, stories, a real number — and the privacy rule asked that prose never reach a prompt. Both hold because every note lives on the row or in the advisor’s own table and is never copied into a need-state. tools/design-privacy-test.js poisons all of them; this checks it on a live deployment.' },

{ id: 'A-74', pass: 'ASK WELL', role: 'advisor', priority: 2, area: 'ASK WELL',
  title: 'Understand — her own words first, the sentence over the map, "Send what I heard", and the way to Compare',
  steps: ['Open Understand and read the first paragraph, then the line under the headline, then the sentence above the island',
          'Press "Send what I heard to j•••@…" as UAT1 on a Journey whose traveller email is a test inbox; read the inbox and the advisor cc',
          'Press "Compare →"; then set the OS to reduce motion and press it again; then disable JavaScript and press it a third time'],
  expect: 'The page opens with the client’s own Finder answers quoted — "You said you need ‘space to think clearly’, and ‘the rainforest’ called you first" — and "With <advisor> · <business>" under the headline. Above the map: "Five places on the island answer moving from running hot toward here, fully — most of them in Nature & Renewal." The mail is from journeys@, to the client, cc and reply-to the advisor, the read-back paragraph and the notes with their labels, no link, no property names; the button then says "Sent just now". Compare → shows the ring and three lines for about two seconds, then Compare; with reduced motion one still frame under a second; with JavaScript off, straight to Compare.',
  why: 'These are the concierge moves — affirm what they said in their words, name who is in the room, send the recap the discovery-call literature asks for, and give the machine a visible moment of thought. Each is theatre only if it lies; the quote is their words, the count is real, the mail is their own answers, and the options were computed before the ring ever turned.' },
