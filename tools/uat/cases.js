...
{ id: 'G-27', role: 'guard', priority: 1, area: 'Build gate',
  title: 'The balance is VISIBLE — the bug that hid the whole feature',
  needs: ['Migration 017 applied'],
  steps: ['Open /hub/campaign with a plan already built',
          'Scroll to the very bottom of the plan',
          'Confirm you see either a count of campaigns left or the offer'],
  expect: 'One of the two. Never the bare sentence "This is your plan. Rebuilding it whenever you like is part of Well Destination Foundations."',
  why: 'That sentence is the fallback for "we cannot read this advisor at all", and for weeks EVERY advisor got it: auth.js did not select plan_builds, so the balance was undefined and D7 never rendered once. If it comes back, the session query has lost a column again — run node tools/session-columns-test.js.' },
...
{ id: 'G-29', role: 'guard', priority: 2, area: 'Build gate',
  ...
  why: 'The builder exists to sell the training; the pack is the downsell. Two prices side by side is a comparison, and $9 against $697 answers itself.' },
