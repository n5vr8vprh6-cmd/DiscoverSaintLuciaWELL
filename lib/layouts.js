...
/* ══════════════════════════════════════════════════════════════════════════
   ...
   extension of Discover Saint Lucia WELL." So this is a fourth layout rather
   than a second application: it uses the same wordmark, the same tokens, the
   same type and the same button vocabulary, and differs only where a working
   surface genuinely needs to differ from an editorial one — denser nav, a
   persistent identity affordance, no marketing footer.

   `page.advisor` is set by the route before rendering. When it is absent the
   page is a signed-out one (login, register, reset) and gets a bare bar, so
   the sign-in screen is not decorated with navigation to places you cannot go.
   ══════════════════════════════════════════════════════════════════════════ */
const HUB_NAV = [
  { label: 'Home',     href: '/hub' },
...
function hubHeader(page) {
  const a = page.advisor;

  if (!a) {
    return `<header class="hub-bar hub-bar--bare">
  <div class="wrap hub-bar-inner">
    ${wordmark({ href: '/' })}
    <span class="hub-bar-context">Travel Advisor Hub</span>
  </div>
</header>`;
  }

  const items = HUB_NAV.map((n) => {
    const current = n.href === page.path ||
  ...
  return `<header class="hub-bar">
  <div class="wrap hub-bar-inner">
    ${wordmark({ href: '/hub' })}
    <nav class="hub-nav" aria-label="Hub">
      ${items}
    </nav>
    <details class="hub-account">
      <summary aria-label="Account menu">
        <span class="hub-avatar" aria-hidden="true">${esc(initials)}</span>
        <span class="hub-account-name">${esc(a.first_name || '')}</span>
      </summary>
      <div class="hub-account-menu">
        <a href="/hub/account">Account settings</a>
        <a href="/">Discover Saint Lucia WELL</a>
        <form method="POST" action="/api/auth/logout" data-hub-logout>
...
/* Deliberately minimal. An advisor working a Journey does not need a sitemap;
   they need the legal links and a way back to the public site. */
function hubFooter() {
  return `<footer class="hub-foot">
  <div class="wrap hub-foot-inner">
    <span>&copy; ${SITE.year} ${esc(SITE.name)}</span>
    <nav aria-label="Hub footer">
