...
const { esc, ringMark, coordMark } = require('./brand.js');
...
function ambientVideo(v, poster) {
  ...
  return `<video class="ambient-video" muted loop playsinline preload="none"${poster ? ` poster="${esc(poster)}"` : ''} aria-hidden="true" tabindex="-1">
        <source src="${esc(v.webm)}" type="video/webm">
...
function lens(s) {
  ...
  return `<section class="section section--paper lens" id="${esc(s.id)}" aria-labelledby="${esc(s.id)}-title">
  <div class="wrap">
    ${sectionHead(s)}
...
function expertise(s) {
  ...
  return `<section class="section section--ink expertise" id="${esc(s.id)}" aria-labelledby="${esc(s.id)}-title">
  <div class="wrap">
    ${sectionHead(s)}
  ...
  const tiles = s.tiles.map((t) => {
    ...
    return `<li class="${t.img ? 'tile--media' : 'tile--type'}"${style}>
        ${t.img ? `<div class="tile-media">${
          t.img.src
            ? `<picture><source srcset="${esc(t.img.src.replace(/\.(jpg|jpeg|png)$/i, ''))}.webp" type="image/webp"><img src="${esc(t.img.src)}" alt="${esc(t.img.alt || '')}" loading="lazy" decoding="async"></picture>`
        ...
        }${ambientVideo(t.video, t.img.src)}</div>` : ''}
        <div class="tile-copy">
...
function finderApp(s) {
  ...
  <div class="wrap">
    ...
    <!-- Interactive quiz, revealed by js/journey.js -->
    <div class="finder-app" id="finder-app" hidden>
      <form class="finder-form" id="finder-form" novalidate>
        <div class="finder-progress" aria-hidden="true"><span class="finder-progress-bar"></span></div>
      ${questions}
        ...
        <p class="finder-privacy">Nothing is stored, and no account is needed.</p>
      </form>
