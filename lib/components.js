...
function lens(s) {
  ...
  /* FOUR ITEMS ORPHAN. .lens-grid is three columns at desktop, so a fourth
     lands alone on a second row — at exactly the width this gets reviewed at.
     Every other lens on the site carries three or six, which is why nobody has
     hit this before.

     Derived from the data rather than set per page, so any future four-item
     lens gets the 2 x 2 for free and nobody has to remember the rule. */
  const four = s.items.length === 4 ? ' lens-grid--four' : '';

  return `<section class="section section--paper lens${seq}" id="${esc(s.id)}" aria-labelledby="${esc(s.id)}-title">
  <div class="wrap">
    ${sectionHead(s)}
    <ul class="lens-grid reveal-group${four}">
      ${items}
    </ul>
...
function compass(s) {
  ...
  const cx = 310, cy = 310, rTick = 168, rTickOut = 190;

  /* THE GAP BETWEEN A TICK AND ITS LABEL IS ONE TICK-LENGTH. Derived rather
     than typed, so the ring, the tick and the label cannot drift apart — and
     so the number carries its own reason. It was a bare 232, which put the
     labels 42 units out and made the drawing wider than it needed to be. */
  const rLabel = rTickOut + (rTickOut - rTick);
  const anyNotes = pts.some((p) => p.note);

  /* Label size lives HERE rather than in CSS because the viewBox below is
     computed from it. Split across two files it would drift, and the symptom
     of drift is a clipped word — which is exactly what happened. */
  const fs = s.labelSize || 21;

  /* Real advance widths, not a guess. Node has no font engine, so this table
     is the font engine's answer written down: uppercase Hanken Grotesk, one
     entry per letter, in em, WITH the 0.14em letter-spacing already in each
     number. Summed across the eight directions it lands within 1.4% of what
     the browser actually lays out, and always slightly OVER — the safe
     direction, since over-padding costs a few empty units and under-padding
     clips a word.

     It replaced a flat 0.84em-per-character bound, which was safe but paid
     16% too much on CELEBRATE — the very word that decides how wide the box
     has to be. Reclaiming that is what let the box become symmetric without
     the ring getting smaller.

     TWO THINGS INVALIDATE THIS TABLE SILENTLY: changing the typeface, and
     changing .compass-label's letter-spacing away from 0.14em. The second is
     guarded in tools/compass-fit.js. Regenerate in DevTools on the homepage:

       const svg = document.querySelector('.compass');
       const t = svg.querySelector('.compass-label').cloneNode(false);
       t.setAttribute('font-size', '100'); t.setAttribute('x', '-9999');
       svg.appendChild(t);
       const out = {};
       'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('').forEach((c) => {
         t.textContent = c.repeat(10);
         out[c] = +(t.getComputedTextLength() / 1000).toFixed(4);
       });
       t.remove(); copy(out);                                              */
  const GLYPH = {
  ...
  /* An unknown character is charged at the widest letter in the set. A label
     with a digit or an accent in it should render slightly padded, never
     slightly clipped. */
  const WIDEST = Math.max(...Object.values(GLYPH));

  function labelWidth(label) {
    return String(label).toUpperCase().split('')
      .reduce((n, c) => n + (GLYPH[c] === undefined ? WIDEST : GLYPH[c]), 0) * fs;
  }


  /* How far each label actually reaches, so the viewBox can be drawn around
     the DRAWING rather than around the ring. See the viewBox block below. */
  const reach = [];
  ...
  const nodes = pts.map((p, i) => {
    ...
    return `<g class="compass-point" data-i="${i}"${p.note ? ` data-note="${esc(p.note)}"` : ''}>
      ...
      <text x="${x.toFixed(1)}" y="${(y + dy).toFixed(1)}" text-anchor="${anchor}" font-size="${fs}" class="compass-label">${esc(p.label)}</text>
      <circle cx="${hx.toFixed(1)}" cy="${hy.toFixed(1)}" r="58" class="compass-hit"/>
    </g>`;
  }).join('\n        ');

  /* THE VIEWBOX IS COMPUTED, NOT CONSTANT, AND SYMMETRIC ABOUT THE RING.
     It began as a flat "0 0 620 620" — the box the RING needs. But the labels
     sit outside the ring by design, so the drawing was always wider than its
     own box and `overflow: visible` let the excess spill. On a desktop it
     spilled into the grid gap and nobody noticed; on a phone the figure IS the
     column, so it spilled off the screen and took CELEBRATE with it (C-21).

     C-21 fixed the clipping by fitting the box to the labels, but fitting each
     side independently left the box LOPSIDED — CELEBRATE reaches 391 units
     west while nothing reaches more than 323 east, so the ring sat at 54.6%
     of the figure rather than 50%. On a mark made entirely of concentric
     circles that reads immediately, even to someone who cannot say why.

     So the half-width is the largest reach in EITHER direction, mirrored. The
     cost is real: about 60 units of deliberate empty space on the east side,
     which is simply what it means to give a lopsided drawing an even box. It
     is paid for by rLabel above — labels one tick-length out instead of two —
     and by the glyph table, which together buy back more than symmetry spends.

     The height stays 0…620. The ring is already centred vertically, and
     leaving it alone keeps the section's rhythm untouched. */
  const half = Math.ceil(Math.max(
    310,
    ...reach.map((r) => cx - r[0]),
    ...reach.map((r) => r[1] - cx)
  )) + 10;
  ...
  <div class="wrap compass-wrap">
    <div class="compass-figure">
      ${compassSvg(pts, s.center)}${readout}${legend}
    </div>
  ...
  /* The ring itself, split out so the Journey Finder's result can show the SAME
     diagram rather than a second one drawn from the same maths. Kept inside
     compass() so the geometry constants above are the only definition of where
     the points sit — two copies of that trigonometry would drift silently, and
     the failure would be a compass whose labels no longer line up with its
     ticks. */
  function compassSvg(points, centre) {
    return `<svg viewBox="${vbX} 0 ${vbW} 620" class="compass" role="img"
           aria-label="The WELL Compass. Eight directions: ${esc(points.map((p) => p.label).join(', '))}.">
        ...
        <text x="${cx}" y="${cy + 44}" text-anchor="middle" class="compass-center">${esc(centre)}</text>
      </svg>`;
  }
}

/* The Finder result needs the ring on its own, with no section, no copy column
   and no hover readout — just the diagram, to be lit up by js/journey.js. It
   goes through compass() so there is exactly one implementation, and the
   wrapper is discarded. */
function compassRing(points, centre, labelSize) {
  const html = compass({ id: 'x', points, center: centre, headline: '', eyebrow: '', labelSize });
  const m = html.match(/<svg[\s\S]*?<\/svg>/);
  if (!m) throw new Error('compassRing: compass() no longer contains an <svg> — the extraction below needs updating');
  return m[0];
}
  ...
  <div class="wrap">
    <div class="split ${s.flip ? 'split--flip' : ''}">
      <div class="split-media">
        ${/* `ui: true` marks a product screenshot rather than a photograph, so
              it gets a frame and the pointer tilt. Photographs bring their own
              edges and must not gain a border — the other six split pages pass
              nothing and are unaffected. */''}
        ${figure(s.img, { className: s.img && s.img.ui ? 'photo--ui' : '' })}
      </div>
...
function credibility(s) {
  /* An empty partner list hides the whole section rather than rendering a
     label over nothing. See the note in content/site.js: the relationships
     are held back pending approval, not withdrawn. */
  if (!s.partners || !s.partners.length) return '';
...
function pageHeader(s) {
  ...
  <div class="wrap">
    ...
    ${/* Optional, so the five other pageHeader pages are untouched. Most page
          headers are an introduction to something you read further down; a few
          are the top of a page whose whole purpose is to get you to act, and
          those should not make you hunt the sticky bar for the button.

          Reuses .hero-ctas and the same gold/ghost pair finalCta puts on a dark
          band — .btn--ghost is currentColor, so it inherits the light text on
          section--ink without a variant of its own. */''}
    ${s.actions && s.actions.length ? `<div class="hero-ctas">
      ${s.actions.map((a, i) => btn(a, i === 0 ? 'gold' : 'ghost')).join('\n      ')}
    </div>` : ''}
  </div>
</section>`;
}
...
/* A property's gallery: the hero large, the rest as a strip of labelled
   thumbnails. Server-rendered and complete without JavaScript — every thumb is
   a link to its own image, which IS the fallback. With JS (js/hub-design.js)
   a tap rebuilds the hero <picture> from the thumb's data-* attributes, the
   same srcset shape mediaPicture() emits, so what swaps in is what the server
   would have rendered. The kind label is a word, never an icon alone.

   The credit line is not decoration. Every frame is a property's own
   photograph, carried with its source and a `cleared` flag that is false
   until Duncan sets it; the count of uncleared frames sits in the caption,
   in the open — the workspace is transparent, and the client document uses
   cleared frames only. */
const KIND_LABEL = {
  hero: 'Overview', spa: 'Spa', room: 'Rooms', lobby: 'Lobby',
  gym: 'Gym', activity: 'Activity', dining: 'Dining', exterior: 'Grounds'
};
function mediaGallery(images, { sizes = '100vw', thumbSizes = '9rem' } = {}) {
  const list = (images || []).filter((i) => i && i.base && i.widths && i.widths.length);
  if (!list.length) return '';
  if (list.length === 1) return `<div class="media-gallery media-gallery--one">${mediaPicture(list[0], { sizes })}</div>`;
  const hero = list[0];
  const credits = list.map((i) => i.credit).filter((c, n, a) => c && a.indexOf(c) === n);
  const ota = list.filter((i) => i.source_kind === 'ota').length;
  const uncleared = list.filter((i) => !i.cleared).length;
  const widest = (i) => i.widths[i.widths.length - 1];
  return `<figure class="media-gallery" data-gallery>
    <div class="media-gallery-hero">${mediaPicture(hero, { sizes })}</div>
    <ol class="media-thumbs">
      ${list.map((img, n) => `<li><a href="${esc(img.base)}-${widest(img)}.jpg" data-thumb data-base="${esc(img.base)}" data-widths="${esc(img.widths.join(','))}" data-alt="${esc(img.alt || '')}"${n === 0 ? ' aria-current="true"' : ''}>${
        mediaPicture(img, { sizes: thumbSizes })}<span>${esc(KIND_LABEL[img.kind] || img.kind)}</span></a></li>`).join('')}
    </ol>
    <figcaption class="media-credit">Photographs: ${esc(credits.join(' · '))}${ota ? ` · ${ota} via booking sites` : ''}${
      uncleared ? ` <span class="media-uncleared">${uncleared} not yet cleared</span>` : ''}</figcaption>
  </figure>`;
}
/* ══════════════════════════════════════════════════════════════════════════
...
/* ══════════════════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════════════════ */
/* A step has three legitimate shapes, and the renderer used to handle two:

     · a `cta`    — something you can do now
     · a `status` — something that exists but is not ready ("In development")
     · neither    — a plain instruction in a sequence

   The third printed the literal string "undefined" into the page, because the
   status branch ran whenever there was no cta and `esc(undefined)` is "undefined".
   It also dimmed the step with `--future`, which keyed off the absence of a cta
   rather than the presence of a status — so an ordinary instruction was styled
   as though it were unavailable.

   Both now key off `status`, which is what actually means "not yet". Nothing on
   /advisors changes: every step there has a cta or a status. */
function pathway(s) {
  const steps = s.steps.map((st, i) => `<li class="path-step${st.status ? ' path-step--future' : ''}">
        <span class="path-n">${String(i + 1).padStart(2, '0')}</span>
        <div class="path-body">
          <h3>${esc(st.title)}</h3>
          <p>${esc(st.text)}</p>
          ${st.cta ? `<a class="contact-link" href="${esc(st.cta.href)}">${esc(st.cta.label)}</a>` : ''}
          ${st.status ? `<span class="path-status">${esc(st.status)}</span>` : ''}
        </div>
      </li>`).join('\n      ');
...
function finderApp(s) {
  ...
  <div class="finder-bar" id="finder-bar" hidden>
    <!-- The strip is full-bleed but its contents sit in the SAME .wrap the
         body uses, so the brand lines up with the headline and Exit lines up
         with the right edge of the content. Giving the bar its own padding
         floated it off to the left of everything below it. -->
    <div class="wrap finder-bar-inner">
  ...
  <div class="wrap">
    ...
    <div class="finder-app" id="finder-app" hidden>
      ...
      <div class="finder-launch finder-card" id="finder-launch">
      ...
      <form class="finder-form finder-card" id="finder-form" novalidate hidden>
      ${questions}
      ...
      <!-- The Compass ring, rendered at build time and parked here for
           js/journey.js to move into the result head and light up. It is the
           SAME diagram as the homepage — compassRing() runs the real component
           — so the directions can never drift apart between the two. -->
      <div class="result-compass-figure" id="finder-compass" hidden>
        <!-- 24 rather than the homepage's 21: the result renders the ring in a
             22rem column, and at 21 the directions turn to grit. The size is
             passed in rather than set in CSS because the viewBox is computed
             from it — see compass(). -->
        ${compassRing(s.compassPoints || [], s.compassCentre || 'YOUR JOURNEY', 24)}
      </div>
...
/* ══════════════════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════════════════ */
/* ══════════════════════════════════════════════════════════════════════════
   PROSE — long-form documents (privacy, terms, accessibility)
   --------------------------------------------------------------------------
   Every other section type on this site is an editorial composition. These
   pages are the opposite: a person arrives wanting one specific fact — what do
   you collect, who gets it, how do I have it deleted — and needs to find it
   without being art-directed at.

   So this renders headings, paragraphs and lists at a strict reading measure
   and nothing else. `body` entries are trusted markup (they are written by us
   in content/, never from input) so a sentence can carry a link.
   ══════════════════════════════════════════════════════════════════════════ */
function prose(s) {
  const blocks = (s.blocks || []).map((b) => {
    const id = b.id ? ` id="${esc(b.id)}"` : '';
    return [
      b.title ? `<h2${id}>${esc(b.title)}</h2>` : '',
      ...(b.body || []).map((p) => `<p>${p}</p>`),
      b.list ? `<ul class="prose-list">${b.list.map((i) => `<li>${i}</li>`).join('')}</ul>` : '',
      b.rows ? `<dl class="prose-rows">${b.rows.map((r) =>
        `<div><dt>${esc(r.term)}</dt><dd>${r.def}</dd></div>`).join('')}</dl>` : ''
    ].filter(Boolean).join('\n      ');
  }).join('\n\n      ');

  return `<section class="section section--paper prose-section" id="${esc(s.id || 'document')}">
  <div class="wrap">
    <div class="prose">
      ${blocks}
      ${s.updated ? `<p class="prose-updated">Last updated ${esc(s.updated)}.</p>` : ''}
    </div>
  </div>
</section>`;
}
...
/* mediaPicture is exported for the Hub's ASK WELL Compare stage, which renders
   the same property photography the consumer directory does — from the same
   image record shape the knowledge bank carries. One helper, two surfaces, so a
   srcset fix lands in both. */
/* ══════════════════════════════════════════════════════════════════════════
   ISLAND MAP — Saint Lucia with the client's villages and places pinned on it
   --------------------------------------------------------------------------
   Server-rendered, complete without JavaScript. The outline is content/
   island.js (geoBoundaries, CC BY 4.0 — the attribution is in <desc> and the
   caption). Every pin is an <a> to that property's card on Compare, which IS
   the JS-off behaviour. Every readout card is rendered here and hidden; js/
   hub-design.js only decides which one is visible — nearest pin to the
   cursor, or the focused one — so the browser moves the server's markup and
   decides nothing.

   Colour is the village accent through the same --v / --v-ink hook the chips
   use, and never carries meaning alone: a lead pin is also larger and
   labelled, a receding pin is smaller and unlabelled, and the legend says the
   words. Two pins at one point (TheLifeCo sits inside A’ILA) are nudged apart
   so both can be pointed at.

   `pins`: [{ slug, name, href, lat, lng, town, approx, village, villageName,
              lead, image, hook }]   `legend`: [{ key, label, lead }]
   ══════════════════════════════════════════════════════════════════════════ */
function islandMap(island, pins, legend, opts) {
  const o = opts || {};
  if (!island || !island.outline) return '';
  const list = (pins || []).filter((p) => p && Number.isFinite(p.lat) && Number.isFinite(p.lng));
  const W = island.width || 600;

  /* Project, then nudge coincident pins apart (within 8 units → 10 units right). */
  const placed = list.map((p) => { const [x, y] = island.project(p.lat, p.lng); return Object.assign({}, p, { x, y }); });
  placed.forEach((p, i) => {
    for (let k = 0; k < i; k++) {
      const q = placed[k];
      if (Math.abs(p.x - q.x) < 8 && Math.abs(p.y - q.y) < 8) { p.x = q.x + 12; p.y = q.y + 6; }
    }
  });

  const v = (p) => (p.village ? ` style="--v: var(--v-${esc(p.village)}); --v-ink: var(--v-${esc(p.village)}-ink)"` : '');
  const labelSide = (x) => (x < W * 0.55 ? { anchor: 'start', dx: 13 } : { anchor: 'end', dx: -13 });

  const towns = (island.labels || []).map((l) => {
    const [x, y] = island.project(l.lat, l.lng);
    const a = l.anchor || labelSide(x).anchor;
    return `<text class="island-town" x="${x + (a === 'end' ? -8 : 8)}" y="${y + 4}" text-anchor="${a}">${esc(l.name)}</text>`;
  }).join('');

  /* Labels for lead pins, kept from overlapping: sorted by y, each label
     sits at least 26 units below the previous one on its side, and a hairline
     leader joins a moved label to its dot. Soufrière holds four of the
     fifteen within a few hundred metres, so this is not hypothetical. */
  const lastY = { start: -Infinity, end: -Infinity };
  placed.filter((p) => p.lead).sort((a, b) => a.y - b.y).forEach((p) => {
    const side = labelSide(p.x);
    let ly = p.y;
    if (ly - lastY[side.anchor] < 26) ly = lastY[side.anchor] + 26;
    lastY[side.anchor] = ly;
    p.labelY = ly; p.side = side;
  });

  /* Receding pins first so lead pins paint on top. */
  const ordered = placed.slice().sort((a, b) => (a.lead ? 1 : 0) - (b.lead ? 1 : 0));
  const pinSvg = ordered.map((p) => {
    const side = p.side || labelSide(p.x);
    const ly = p.labelY == null ? p.y : p.labelY;
    const moved = Math.abs(ly - p.y) > 2;
    return `<a href="${esc(p.href || '#')}" class="island-pin${p.lead ? ' is-lead' : ''}" data-pin="${esc(p.slug)}"${v(p)} tabindex="0" aria-label="${esc(p.name)}${p.town ? ', ' + esc(p.town) : ''}">
        <circle class="island-hit" cx="${p.x}" cy="${p.y}" r="28"></circle>
        ${p.lead && moved ? `<path class="island-leader" d="M${p.x} ${p.y}L${p.x + side.dx * 0.6} ${ly}L${p.x + side.dx} ${ly}"></path>` : ''}
        <circle class="island-dot" cx="${p.x}" cy="${p.y}" r="${p.lead ? 10 : 6.5}"></circle>
        ${p.lead ? `<text class="island-name" x="${p.x + side.dx + 3}" y="${ly + 7}" text-anchor="${side.anchor}">${esc(p.name)}</text>` : ''}
      </a>`;
  }).join('');

  /* `story` is the advisor's own note about the place; `opts.extra(p)` lets
     the caller append its own markup (an edit form) to each card. */
  const extra = typeof o.extra === 'function' ? o.extra : () => '';
  const cards = placed.map((p, i) => `<div class="island-card" data-pin-card="${esc(p.slug)}"${v(p)}${i === 0 && !placed.some((q) => q.lead) || p === placed.filter((q) => q.lead)[0] ? '' : ' hidden'}>
      ${p.image ? `<div class="island-card-media">${mediaPicture(p.image, { sizes: '14rem' })}</div>` : ''}
      <div class="island-card-copy">
        <p class="island-card-village">${esc(p.villageName || '')}</p>
        <p class="island-card-name"><a href="${esc(p.href || '#')}">${esc(p.name)}</a></p>
        ${p.town ? `<p class="island-card-town">${esc(p.town)}${p.approx ? ' <span class="island-approx">· placed from its address</span>' : ''}</p>` : ''}
        ${p.hook ? `<p class="island-card-hook">${esc(p.hook)}</p>` : ''}
        ${p.story ? `<p class="island-card-story" data-fragment-slot="story-${esc(p.slug)}"><span class="island-card-storywho">Your note</span> ${esc(p.story)}</p>` : `<p class="island-card-story" data-fragment-slot="story-${esc(p.slug)}" hidden></p>`}
      </div>
      ${extra(p)}
    </div>`).join('');

  const legendHtml = (legend || []).length ? `<ul class="island-legend chips" aria-label="Villages">${legend.map((l) => `
      <li class="chip chip--v${l.lead ? ' chip--lead' : ''}" style="--v: var(--v-${esc(l.key)}); --v-ink: var(--v-${esc(l.key)}-ink)">${esc(l.label)}</li>`).join('')}</ul>` : '';

  const titleId = 'island-title-' + Math.abs(hash(placed.map((p) => p.slug).join('|')));
  return `<div class="island" data-island>
    <div class="island-figure">
      <svg class="island-svg" viewBox="${esc(island.viewBox)}" role="img" aria-labelledby="${titleId}" focusable="false">
        <title id="${titleId}">${esc(o.title || 'Saint Lucia, with the places on this shortlist')}</title>
        <desc>${esc(island.attribution || '')}. Pins are the places we are considering, coloured by the village each answers for this traveller.</desc>
        <path class="island-land" d="${island.outline}"></path>
        ${towns}
        ${pinSvg}
      </svg>
      ${o.caption === false ? '' : `<p class="island-caption"><span class="coords">13°54′N 60°58′W</span><span class="island-attrib">${esc(island.attribution || '')}</span></p>`}
    </div>
    <div class="island-side">
      <div class="island-readout" aria-live="polite">${cards || '<p class="design-empty">No places on the shortlist yet.</p>'}</div>
      ${legendHtml}
    </div>
  </div>`;
}
...
module.exports = { renderSections, RENDERERS, figure, btn, sectionHead, paras, esc, mediaPicture, mediaGallery, islandMap, KIND_LABEL };
