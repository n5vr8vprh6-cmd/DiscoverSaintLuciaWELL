...
/* A property's gallery: the hero large, the rest as a strip of labelled
   ...
   until Duncan sets it; the count of uncleared frames is advisor-only
   (data-hide-in-present) because the client is not shown our paperwork. */
const KIND_LABEL = {
...
function mediaGallery(images, { sizes = '100vw', thumbSizes = '9rem' } = {}) {
  ...
  return `<figure class="media-gallery" data-gallery>
    ...
    <figcaption class="media-credit">Photographs: ${esc(credits.join(' · '))}${ota ? ` · ${ota} via booking sites` : ''}${
      uncleared ? ` <span class="media-uncleared" data-hide-in-present>${uncleared} not yet cleared</span>` : ''}</figcaption>
  </figure>`;
}
/* ══════════════════════════════════════════════════════════════════════════
...
module.exports = { renderSections, RENDERERS, figure, btn, sectionHead, paras, esc, mediaPicture, mediaGallery, KIND_LABEL };
