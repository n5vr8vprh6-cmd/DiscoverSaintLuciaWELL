...
function compass(s) {
  ...
  /* How far each label actually reaches, so the viewBox can be drawn around
     the DRAWING rather than around the ring. See labelBox(). */
  const reach = [];
  ...
  /* An upper bound on the advance width of an uppercase Hanken Grotesk label,
     in viewBox units. MEASURED, not guessed: getBBox() on the live homepage
     compass at font-size 21 gives 15.2–17.6 units per character across the
     eight directions (the high end is MOVE, whose M and W are the widest
     glyphs in the set). 0.84em covers all of them, letter-spacing included.

     It is an estimate because Node cannot lay out SVG text, and a small
     over-estimate is the safe direction to be wrong in — it costs a few units
     of empty margin, where an under-estimate costs a clipped word. Verify by
     re-running that getBBox() measurement and confirming every label still
     sits inside the viewBox; tools/compass-fit.js prints the numbers. */
  function labelWidth(label) { return String(label).length * fs * 0.84; }

  /* THE VIEWBOX IS COMPUTED, NOT CONSTANT — this is the C-21 fix.
     It used to be a flat "0 0 620 620", which is the box the RING needs. But
     the labels are placed outside the ring by design, so the drawing has always
     been wider than its own viewBox, and `overflow: visible` let the excess
     spill out of the figure. On desktop it spilled into the grid gap and nobody
     noticed. On a phone the figure IS the column, so it spilled off the side of
     the screen and took CELEBRATE with it — the longest word, sitting on the
     left where the overhang is worst (measured: 59 units past the edge).

     Widening the box means the ring is drawn a little smaller for the same
     container width. That is the trade, and it is the right way round: a
     slightly smaller compass that can be read entirely beats a full-width one
     with a word cut off. The figure max-widths in css/site.css are raised to
     match, so the ring keeps close to its former size wherever there is room. */
  const vbX = Math.floor(Math.min(0, ...reach.map((r) => r[0])) - 10);
  const vbR = Math.ceil(Math.max(620, ...reach.map((r) => r[1])) + 10);
  const vbW = vbR - vbX;
